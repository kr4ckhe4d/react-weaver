...
const ReactWeaverApp: React.FC = () => {
  const { generateCode } = useDesign(); // Removed isLoadingAi
  const { toast } = useToast();
  const codeDialogRef = useRef<CodePreviewDialogRef>(null);

  const handleGenerateCode = async () => {
    // Toast message can be updated to reflect that AI is not used or just a generic message
